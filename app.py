...
if 'recommended_genre' not in st.session_state:
    st.session_state.recommended_genre = None # 추천 장르 초기화

# 버튼 클릭 이벤트 처리
if st.button('추천 장르 받기'):
    st.session_state.recommended_genre = st.session_state.model.select_arm() # 추천 장르 선택
    with st.spinner('추천할 장르를 찾는 중...'):
        st.session_state.recommended_link = crawling.get_soundcloud_links(st.session_state.recommended_genre['Genre']) # 추천 장르에 대한 링크 가져오기

if st.session_state.recommended_genre is not None:
    st.write(f"추천 장르 : {st.session_state.recommended_genre['Genre']}") # 추천 장르 출력
    print(st.session_state.recommended_link) # 링크 출력
    iframe_html = f"""
        <iframe
            width="100%"
            height="166"
            ...
            frameborder="no"
            allow="autoplay"
            src="https://w.soundcloud.com/player/?url={st.session_state.recommended_link}&color=%23ff5500&auto_play=false&hide_related=false&show_comments=true&show_user=true&show_reposts=false&show_teaser=true">
        </iframe>
    """ # SoundCloud iframe HTML 생성
    components.html(iframe_html, height=166) # iframe HTML을 streamlit에 삽입
    user_input = st.slider("점수 (1~10)", 1, 10) # 사용자 점수 가져오기
    if st.button('점수 제출'):
        st.session_state.model.update(list(st.session_state.recommended_genre.values())[1:], user_input) # 모델 업데이트
