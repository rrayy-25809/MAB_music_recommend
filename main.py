...
class Music_MAB: #멀티 암드 밴딧 알고리즘을 담은 클래스 생성
    ...
    def update(self, reward_vector:list[float], user_input:int):
        if self.epsilon == 1: # 처음 탐색 비율이 1일 때
            ...
            self.user_status += (np.array(reward_vector) - self.user_status) * ((user_input-5)/10) # 현재 상태와 reward_vector 간의 차이를 user_input/10 비율만큼 반영
        if self.epsilon < 0.1:
            self.epsilon = 1 # 탐색 비율이 0.1보다 작아지면 1로 초기화
